...
export class tabUnit {
  ...
  async createChild(el, placeString = "data.sub", unit?, slot?, user?) {
    let parentType, structure = { settings: { unit: null }, dataType: null }, { key } = this.api().parsePlaceString(placeString);
    try {
      // @ts-ignore
...
export class tabModule {
  ...
  async get(id: string, user?) {
    ...
    return Promise.reject({ status: "FORBIDDEN", message: (user ? user.username + "#" + user._id : "匿名用户") + "没有查看id为 #" + id + " " + this.className + "的权限" });
  }
