...
// const {reactive} = require('../reactivity/index')
...
export class tabUnit {
    ...
    async refresh(user?) {
        return this.api().getFromController(this._id, user);
    }
    ...
    async getType(user?) {
        return this.api().getType(this.type, user);
    }
    ...
    async createChild(el, placeString = "data.sub", unit = this.className, slot = "Array", user?) {
        if (slot === "Object") {
...
export class tabModule {
    ...
    assign(el) {
        ...
        if (element && element._id) {
            ...
            this.list[id] = reactive(new this.unit(el));
    ...
    async get(id: string, user?) {
        ...
        return Promise.reject({status: "FORBIDDEN", message: "您没有查看id为 #" + id + " " + this.className + "的权限"});
    }
    ...
    async save(el, user?) {
        ...
        return Promise.reject({status: "FORBIDDEN", message: "您没有修改" + this.className + " #" + el._id + " 的权限"});
    }
    ...
    rights(el, user?) {
        ...
        if (user) {
            if (!isClient && user._id === this.tab.config.users.root) {
                isRoot = true;
            }
